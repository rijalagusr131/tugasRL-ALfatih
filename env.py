...
class Environment(tk.Tk, object):
    ...
    def build_environment(self):
        ...
        for column in range(0, env_width * pixels, pixels):
            x1, y1 x0, y0 = column, 0, column, env_height * pixels
            self.canvas_widget.create_line(x1, y1, x0, y0, fill='grey')
        for row in range(0, env_height * pixels, pixels):
            x1, y1, x0, y0 = 0, row, env_height * pixels, row
            self.canvas_widget.create_line(x1, y1, x0, y0, fill='grey')
        ...
        self.bank1_object = ImageTk.PhotoImage(img_flag)
        self.bank1 = self.canvas_widget.create_image(pixels * 6, pixels * 6, anchor='nw', image=self.bank1_object)
